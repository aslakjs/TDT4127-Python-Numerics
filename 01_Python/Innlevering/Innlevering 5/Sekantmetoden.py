...
def differentiate(x_k, x_k1, func):
    if func == f:
        afx = ( f(x_k) - f(x_k1) ) / ( x_k - x_k1 )
    elif func == g:
        afx = ( g(x_k) - g(x_k1) ) / ( x_k - x_k1 )
    return afx
...
def secant_method(x0, x1, func, tol):
    ...
    while ( abs(func(x1)) > tol):
        y = x1 - func(x1)*((differentiate(x0, x1, func))**(-1))
        x0 = x1
        x1 = y
        it += 1
        
    if func == f:
        print(f'Funksjonen nærmer seg et nullpunkt når x = {round(x1,2):.2f} , da er f(x) =  {f(x1):.2e}')
    elif func == g:
        print(f'Funksjonen nærmer seg et nullpunkt når x = {round(x1,2):.2f} , da er g(x) =  {g(x1):.2e}')
